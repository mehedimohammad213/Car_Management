...
export class InvoiceService {
  ...
  private static createPDF(data: InvoiceData): void {
    ...
    addText(data.company.name, 50, 25, { fontSize: 20, color: '#ffffff', style: 'bold' });
    ...
    // Invoice details
    addText('INVOICE', pageWidth - 60, 25, { fontSize: 24, color: '#ffffff', style: 'bold' });
    addText(`#${data.invoice_number}`, pageWidth - 60, 32, { fontSize: 14, color: '#e2e8f0' });
    addText(`Date: ${data.invoice_date}`, pageWidth - 60, 40, { fontSize: 10, color: '#e2e8f0' });
    addText(`Due: ${data.due_date}`, pageWidth - 60, 45, { fontSize: 10, color: '#e2e8f0' });
    ...
    if (data.order.user) {
      addText(data.order.user.name, 20, yPosition, { fontSize: 12, style: 'bold' });
      yPosition += 6;
      addText(data.order.user.email, 20, yPosition, { fontSize: 10, color: secondaryColor });
      yPosition += 6;
    }
    
    if (data.order.shipping_address) {
      addText('Shipping Address:', 20, yPosition, { fontSize: 10, style: 'bold' });
      yPosition += 6;
    ...
    addText('Qty', pageWidth - 80, yPosition + 2, { fontSize: 10, style: 'bold' });
    addText('Price', pageWidth - 60, yPosition + 2, { fontSize: 10, style: 'bold' });
    addText('Total', pageWidth - 30, yPosition + 2, { fontSize: 10, style: 'bold' });
    ...
    data.order.items.forEach((item, index) => {
      ...
      // Item details
      addText(`${item.car.make} ${item.car.model}`, 40, itemY, { fontSize: 10, style: 'bold' });
      addText(`Year: ${item.car.year}`, 40, itemY + 5, { fontSize: 8, color: secondaryColor });
      addText(`Mileage: ${item.car.mileage_km?.toLocaleString()} km`, 40, itemY + 10, { fontSize: 8, color: secondaryColor });
      
      // Quantity
      addText(item.quantity.toString(), pageWidth - 80, itemY + 5, { fontSize: 10, style: 'bold' });
      
      // Price
      addText(`$${item.price.toLocaleString()}`, pageWidth - 60, itemY + 5, { fontSize: 10 });
      
      // Total
      const itemTotal = item.price * item.quantity;
      addText(`$${itemTotal.toLocaleString()}`, pageWidth - 30, itemY + 5, { fontSize: 10, style: 'bold' });
    });
    ...
    addText(`$${subtotal.toLocaleString()}`, totalsX + 50, yPosition, { fontSize: 12, style: 'bold' });
    yPosition += 8;

    addText('Tax (8%):', totalsX, yPosition, { fontSize: 12 });
    addText(`$${tax.toLocaleString()}`, totalsX + 50, yPosition, { fontSize: 12, style: 'bold' });
    yPosition += 8;

    addText('Shipping:', totalsX, yPosition, { fontSize: 12 });
    addText(shipping === 0 ? 'Free' : `$${shipping.toLocaleString()}`, totalsX + 50, yPosition, { fontSize: 12, style: 'bold' });
    yPosition += 8;
    ...
    addText(`$${total.toLocaleString()}`, totalsX + 50, yPosition, { fontSize: 14, style: 'bold', color: accentColor });
    yPosition += 20;

    // Order status
    const statusColor = this.getStatusColor(data.order.status);
    addText('Order Status:', 20, yPosition, { fontSize: 12, style: 'bold' });
    addText(data.order.status.toUpperCase(), 80, yPosition, { fontSize: 12, style: 'bold', color: statusColor });
    yPosition += 10;
    ...
    addText('Thank you for your business!', 20, yPosition, { fontSize: 12, style: 'bold', color: primaryColor });
    yPosition += 8;

    addText(`For questions about this invoice, contact us at ${data.company.email}`, 20, yPosition, { fontSize: 10, color: secondaryColor });
    addText(`Visit us at ${data.company.website}`, 20, yPosition + 6, { fontSize: 10, color: secondaryColor });

    // Page number
    addText(`Page 1 of 1`, pageWidth - 30, pageHeight - 10, { fontSize: 8, color: secondaryColor });
